...
export default function UserList() {
    ...
    const columns = [
        ...
        },
        {
            title: '用户状态',
            dataIndex: 'roleState',
            render: (roleState, row) => <Switch checked={roleState} disabled={row.default}></Switch>
        },
        {
            title: '操作',
            render: (row) => {
                return <Space>
                    <Button disabled={row.default} type='link' icon={<EditOutlined />}>编辑</Button>
                    <Button disabled={row.default} danger type='link' icon={<DeleteOutlined />} onClick={() => confirmMethod(item)}>删除</Button>
                </Space>
            }
        },
    ]
