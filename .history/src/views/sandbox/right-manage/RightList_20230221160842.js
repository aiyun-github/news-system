    ...
    const columns = [
        ...
        },
        {
            ...
            render: (item) => {
                return <Space>
                    {/* 气泡卡片 */}
                    <Popover content={<div style={{textAlign: 'center'}}><Switch checked={item.pagepermisson}></Switch></div>} title="页面配置项" trigger={item.pagepermisson ? 'click' : ''}>
                        <Button type='link' icon={<EditOutlined/>}  disabled={!item.pagepermisson} onChange={() => switchMethod(item)}>编辑</Button>
                    </Popover>
